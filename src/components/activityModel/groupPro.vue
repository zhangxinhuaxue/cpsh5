            ...
            <div class="infos">
                <h3>{{pro.productName?pro.productName:"已下架"}}</h3>
                    <p class="price">
                        ...
                        <span class="s3" :style="{color: '#' + tabInfo.limitTextColor}">({{limitTxt[0]}}<i>{{pro.limit}}</i>{{limitTxt[1]}})</span>
                    </p>
                    <p class="origin-price">{{origin}}</p>
    ...
    export default {
        ...
        data() {
            return {
                ...
                origin: this.tabInfo.origPriceText + ':¥' + formatMoney(this.pro.origPrice)
            }
        },
        props: {
            pro: {},
        ...
        mounted: function() {
            ...
            if (this.start < this.sysTime && this.end > this.sysTime) { // 团购中
                if (this.pro.left > 0) { // 产品一没有卖完
                    this.tipStyle = {
                ...
                } else if (this.pro.left <= 0 && this.pro.left2 > 0) { // 产品一卖完，产品二未卖完
                    this.tipStyle = {
                    ...
                    this.url = '/mpages/mall/product/' + this.pro.pid2
                    this.pro.limit = this.pro.limit2
                    this.tabInfo.priceText = this.tabInfo.origPriceText
                    this.price = formatMoney(this.pro.origPrice)
                    this.origPrice = ''
                    this.origin = ''
                } else { // 全部卖完
                    this.tipStyle = {
            ...
            } else if (this.start > this.sysTime) { // 团购前
                this.tipStyle = {
                ...
                this.url = '/pages/mall/product/' + this.pro.pid
            } else if (this.end < this.sysTime) { // 团购结束
                if (this.pro.left2 > 0) { // 团购时间结束，产品换为产品二&&产品二未卖完
                    this.tipStyle = {
                        color: formatColor(this.tabInfo.afterTextColor),
                        backgroundColor: formatColor(this.tabInfo.afterTextBgColor)
                    }
                    ...
                    this.btnTxt = this.tabInfo.afterBtnText
                    this.url = '/pages/mall/product/' + this.pro.pid2
                } else { // 全部卖完
                    this.tipStyle = {
                        color: '#ffffff',
                        backgroundColor: '#bbbbbb'
