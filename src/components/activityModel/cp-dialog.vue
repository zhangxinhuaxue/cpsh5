<!-- 红包弹窗模块，与coupon-dialog不同，这个是cms配置的红包模块专用的弹窗 -->
<template>
    <div>
        <div class="hb-dialog2" v-show="config.show">
            <div class="hb-box" :class="{ 'hb-box-show': config.show }">
                ...
                <!-- 红包壳 -->
                <div class="hd-shell">
                    <p>{{config.couponTop}}</p>
                    <p v-html="config.couponTips"></p>
                    <div v-if="config.showBtn" class="hb-close-btn" @click="gotoUseIt">
                        {{config.btnTxt}}
                    </div>
                </div>
            </div>
    ...
    methods: {
        gotoUseIt() {
            ...
            if (window.aixuedai.isUavoApp === 'y') {
                window.location.href = this.config.couponUrl || '/pages/coupon/my/list'
            } else {
                window.location.href = this.config.couponUrl || 'http://www.aiyoumi.com/m'
            }
        },
        closeDialog() {
            this.config.show = false
