...
const defaultStyle = [
    ...
    ' .time-block {display: inline-block; width: 14px; height: 20px; text-align: center; line-height: 20px; background-color: rgba(0, 0, 0, 0.6); color: #fff; margin: 0 1px;}',
    ' .ms-block{background-color:green}'
]
...
class Seckill {
    constructor(options) {
        let pId = '#seckill-' + new Date().getTime()
        Object.assign(
            this,
            {
                style: defaultStyle.map((item) => pId + item).join(''),
    ...
    destory() {
        ...
        // 移除dom
        if (this._pElem) {
            this._pElem.parentNode.removeChild(this._pElem)
            this._pElem = null
        }
    }
    init() {
    ...
    _getGap() {
        return new Promise((resolve, reject) => {
            this.getServerTime
                ? this.getServerTime().then(({ success, serverTime }) => {
                    if (success) {
    ...
    _mount(template, el, pClass) {
        if (!this._pElem) {
            ...
            pElem.id = this._pId.slice(1)
            pElem.className = !pClass ? '' : pClass
            el.appendChild(pElem)
            this._pElem = pElem
        }
        this._pElem.innerHTML = template
    }
    ...
    _compileDisplay(template) {
        ...
        while (reg.test(template)) {
            ...
            // 包含或运算
            if (regexp1) {
                let stateArray = regexp1
                    .slice(0, regexp1.length - 2)
                    .replace(/\$/g, '')
                    .split('||')
