...
export function goBiopsyBankcardV3(params, gid, tid) {
    axdApp.send({
        ...
        callback: (res) => {
            if (res.code == -995) {
                window.location.href = '/pages/vipcard-2022'
                return
            }
...
// 获取收银台参数
export function getPaymentParamsV3(gid, tid, returnUrl, backUrl) {
    let quitUrl = `${location.origin}/pages/trade/pay-error?backUrl=${encodeURIComponent(backUrl)}`
    returnUrl = returnUrl || `${location.origin}/pages/trade/payResult/${tid}?gid=${gid}`
    return new Promise((resolve, reject) => {
        getPayParameterByGid({
            gid,
            quitUrl,
...
export var payMixinMulti = {
    methods: {
        createOrder({params = {}, callback, returnUrl, backUrl}= {}) {
            this.comfirmFunction({ params: JSON.stringify(params) }).then((res) => {
                if (res.success) {
                    ...
                    // 获取收银台参数
                    getPaymentParamsV3.call(this, gid, tradeid, returnUrl, backUrl)
                        .then(res => {
                            // 唤起收银台
